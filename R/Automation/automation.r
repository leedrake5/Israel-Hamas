...
create_by_type <- function(country, date=NULL) {
    ...
    url = "https://armadarotta.blogspot.com/2023/10/israel-at-war-tracking-equipment-losses.html"
  ...
  heads <- heads[nchar(heads) > 0 & substring(heads, 1, 1)!="(" & substring(heads, 1, 1)!=":"]
  heads_length <- length(heads)

  # Get the positons of the Russia and Ukraine headers
  pos <- which(substring(heads, 1, 27)=="Trucks, Vehicles, and Jeeps")[1]
  
  if(country=="Israel"){
      heads <- heads[1:pos]
      pos <- length(heads)
  } else if(country=="Hamas"){
      heads <- as.vector(na.omit(heads[pos+1:length(heads)]))
      pos <- length(heads)
  }
  ...
  country_df <- totals %>%
    ...
    adorn_totals("row") %>%
    #dplyr::mutate(
    #  equipment = replace(equipment, pos+1, "All Types"),
    #) %>%
    dplyr::rename(equipment_type = equipment)
    country_df$equipment_type <- gsub(":", "", country_df$equipment_type)
    country_df$equipment_type[pos+1] <- "All Types"
    country_df$country[pos+1] <- country
...
totals_by_type <- function(date=NULL) {
  israel <- create_by_type("Israel", date=date)
  hamas <- create_by_type("Hamas",date=date)

  totals_df <- israel %>%
    dplyr::bind_rows(hamas, .id=NULL)
...
totals_fold <- function(totals_df=NULL, link="https://armadarotta.blogspot.com/2023/10/israel-at-war-tracking-equipment-losses.html", date=NULL){
...
googleSheetPush <- function(results, sheet_url="https://docs.google.com/spreadsheets/d/1uxCivR-Dc7AiBUNKOWEarmb4zaoA11kEgSAt8vO1zUs"){
...
daily_update <- function(link="https://armadarotta.blogspot.com/2023/10/israel-at-war-tracking-equipment-losses.html", to_return=NULL){
...
link="https://armadarotta.blogspot.com/2023/10/israel-at-war-tracking-equipment-losses.html"
url = link
